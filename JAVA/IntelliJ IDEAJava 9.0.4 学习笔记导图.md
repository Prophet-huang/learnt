...
对象 object：根据所需属性，引用的类, 的抽象说法(一个容器存着数据)
...
- 属于类的属性
- 与实例一样形式类似是，成员变量，常量，这些信息储存器的现实体现。
- 信息储存器只是声明可以说字段，当被赋予值时就根据需求变成对应种类的变量了，此时就不能说字段了。
...
- 写构造方法不需要返回值，不是空返回值因此是没有类型的，通常情况下需要一个无参数的构造，来满足初始化时不填参数，或给参数设置默认值：数值类型默认`0`，布尔类型默认`false`,对象引用`null`。
- 构造方法的使用：初始化对象。

- 使用构造方法后封装失效的问题
- 如果构造器低一个语句是`this(...)`，这个构造器将会调用该类的另一个构造器。



### 初始化块`initialiazation block`

无论使用那个构造器构造对象，都会优先在初始化块中初始化。不过这种方式不常用一般都用构造方法。

**注意：类在第一次加载时最先初始化静态代码块。**
...
静态常量：常量的基础上，它属于类不属于任何对象，可由类直接调用。
...
- 类层面的方法，可由类直接调用。
- 方法不需要访问对象状态，常用于功能性。(例如：Math.pow `将第一个参数的值返回到第二个参数的幂(平方)`)
- 常配合静态字段和单例设计模式使用
...
工厂模式是实例化对象的一种模式，是用工厂方法代替构造对象(new操作)的一种模式。

本质上是将不变的部分提取出来，将可变的部分留作接口，以达到最大程度上的复用。



### 原生方法

非`java`语言实现，可绕过`java`语言的访问控制机制 

例如： `System`类中又个`setOut`方法可以将 `System.out` 方法的输出流改变，即使字段`out`是`final`修饰的。



### 关于传参

切记在**非**浅拷贝情况下引用数据类型的变量名存的是对象的地址，对象开辟储存空间储存信息。

可传两种参数：

- 参数是基本数据类型：方法没法改变变量的参数，使用时只是使用其副本，如要非要改变需方法中设置返回值并后续重新赋值。（按变量的参数调用，不改变变量本身）
- 参数是引用数据类型：方法可以改变对象的参数，使用时是对象参数的副本，对象参数的副本是浅拷贝因此副本变量是指向**原对象的*变量***，副本变量和原对象变量最终都指向一个对象，调用同一对象的参数。（按引用对象的参数调用）

另外：

- 对象参数之间是可以通过方法传参交换的。
- 方法传递引用类型不能改变其**对象**，因为在方法中只是改变副本变量的引用对象，和原对象的变量没关系，例如：两个对象完成交换。
...
包装类比较对象中的元素要用 equals()；方法，不能直接用 ==（比较地址），`equals`方法在`Object`类中和`==`一样都是比较地址（`==`仅限于比较基础类型），但在包装类中`equals`会被重写，使其对比不是地址而是实际的内容。例如：

```java
// 源码中包装类Long 的 Long.equals 源码
    
private final long value; // Long对象 会被拆箱成 long类型

// 重写equals方法
public boolean equals(Object obj) {
    	// 先判断传入参数是不是Long类型不是直接false，instanceof java保留字 判断前后类型是否一致
        if (obj instanceof Long) {
            // 将传入参数转化为long基础类型然后比较，源码Object类型无法调用 LongValue()；方法需要强转。
...
**但是**在 BigDeciaml 中需要比较到元素数据的数学性质时，要注意不能使用 equals(); 要使用 compareTo(); 方法，因为元素的小数位数的不同会导致元素的内容不同，但在数学上：18.000 = 18.0 ，不能因为小数内容干扰是否相等的判断。
